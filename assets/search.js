...
const SEARCH_INDEX = [
  ...
  { title: 'Tips / Kat\'s Hacks',     snippet: 'Practical life hacks from Kat — everything I wish someone had told my friends.', url: '__know_lifehacks', icon: '💡', tag: 'Things to Know', keywords: 'tips life hacks advice' },
...
// ------------------------------------------------------------
//  Full-text index — data/search-index.json
//  Generated by _builder/build_search_index.py. Covers the real
//  body text of every page, so a query like "peking duck" finds
//  guides/beijing.html even though that phrase lives in the page
//  content. Loaded lazily; if the file is missing, search simply
//  degrades to the curated index above + cities, as before.
// ------------------------------------------------------------
let _rcFulltext = null;

function rcLoadFulltext() {
  if (_rcFulltext !== null) return;
  fetch(_rcRoot + 'data/search-index.json')
    .then(r => { if (!r.ok) throw new Error('no fulltext index'); return r.json(); })
    .then(items => { _rcFulltext = items; })
    .catch(() => { _rcFulltext = []; });
}

function rcMatchFulltext(q, excludeUrls) {
  if (!_rcFulltext || !_rcFulltext.length) return [];
  const terms = q.toLowerCase().split(/\s+/).filter(Boolean);
  if (!terms.length) return [];
  const out = [];
  for (const item of _rcFulltext) {
    const text = (item.text || '').toLowerCase();
    if (!terms.every(t => text.includes(t))) continue;
    if (excludeUrls.has(item.url)) continue; // curated entry already shows this page
    out.push({
      title: item.title || item.url,
      snippet: rcSnippetFrom(text, rcFirstHit(text, terms)),
      url: item.url,
      icon: '📄',
      tag: item.tag || 'Page',
    });
...
function rcFirstHit(text, terms) {
  let best = -1;
...
function rcSnippetFrom(text, hitIdx) {
  if (hitIdx < 0) return text.slice(0, 140) + (text.length > 140 ? '…' : '');
  const start = Math.max(0, hitIdx - 45);
  const end = Math.min(text.length, hitIdx + 65);
...
function rcInitSearch() {
  ...
  // Load the full-text index (covers every page's real content)
  rcLoadFulltext();
}
...
function rcRunSearch(query) {
  ...
  // Full-text matches for anything not already covered above
  const excludeUrls = new Set();
  pageMatches.forEach(r => excludeUrls.add(r.url));
  cityMatches.forEach(r => excludeUrls.add(r.url));
  const fulltextMatches = rcMatchFulltext(q, excludeUrls);

  _rcResults = [...pageMatches, ...cityMatches, ...fulltextMatches];
